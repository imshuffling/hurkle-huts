...
export default function BlockVideo({
  ...
  return (
    <div className='section video w-full relative'>
      <div className='absolute text-white drop-shadow-xl z-[1] text-center top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full md:w-1/2 px-6'>
        {!hideBlockTitle && <h3 className='text-center p-2'>{title}</h3>}
        {text && renderRichText(text)}
      </div>

      <div className='absolute z-8 top-0 right-0 bg-gradient-to-b from-[#231f20] h-full w-full' />
